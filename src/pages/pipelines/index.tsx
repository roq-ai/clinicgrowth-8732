  ...
  const fetcher = useCallback(
    async () =>
      getPipelines({
        ...
        order: params.order,
        searchTermKeys: ['stage.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PipelineInterface>>(
    () => `/pipelines?params=${JSON.stringify(params)}`,
    fetcher,
  );
    ...
    hasAccess('lead', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/leads/view/${record.lead?.id}`}>
              {record.lead?.name}
            </Link>
          ),
        }
